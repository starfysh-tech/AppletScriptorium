...
- **SwiftHAL**: Halstead complexity metrics analyzer for Swift code with visual TUI, architecture insights, and CI/CD integration.
...
- Mail rule conditions (ALL must be met):
...
- Status log: `runs/summarizer-status.log` records one line per run with metrics (timestamp, from, subject, counts, status)
